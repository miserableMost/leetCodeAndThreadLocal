...
public class Method {
    ...
    public ArrayList<ArrayList<Integer>> findLoopPath(){
        ...
        while (iterator.hasNext()){
            ...
            if(node.getIndegree()==0){
                ...
                path(lists,new ArrayList<>(),node,node,1,true);
                //起始节点查找一次后，就可移除
                node.setIndegree(0);
                //fnc(node.getToSet());
            }
    ...
    private void path(ArrayList<ArrayList<Integer>> lists, ArrayList<Integer> list, Node fromnode, Node start, int depth, boolean in){
        //深度大于7返回
        if(depth>7 || fromnode.getIndegree()<1 ) return;
        list.add(fromnode.getVal());
        if(in && !start.equals(fromnode)) {
            if(fromnode.getIndegree() == 1)
                fromnode.setIndegree(0);
            else
                in = false;
        }
        //当前路径总入度
//        path_indegree_total += fromnode.getIndegree();
        ...
        while (iterator.hasNext()){
            Node node = (Node) iterator.next();
            path(lists,list,node,start,depth+1,in);
        }
