...
public class AllCyclesInDirectedGraphTarjan {
    ...
    public List<List<Vertex<Integer>>> findAllSimpleCycles(Graph<Integer> graph) {
        ...
        for(Vertex<Integer> vertex : graph.getAllVertex()) {
            findAllSimpleCycles(vertex, vertex, result,1);
            visited.add(vertex);
    ...
    private boolean findAllSimpleCycles(Vertex start, Vertex<Integer> current,List<List<Vertex<Integer>>> result,int depth) {
        //找到环
        if(depth > 7)
           return false;
        ...
        for (Vertex<Integer> w : current.getAdjacentVertexes()) {
            if (visited.contains(w)) {
                continue;
            } else if (depth > 2 && w.equals(start)) {
                hasCycle = true;
