...
public class Method {
    ...
    public ArrayList<ArrayList<Integer>> findLoopPathByMultiThread() throws InterruptedException {
        ArrayList<ArrayList<Integer>> lists = new ArrayList<>();
        TreeSet<Node> thread1_set = new TreeSet<>();
        TreeSet<Node> thread2_set = new TreeSet<>();
        int mid = set.size()/2;
        for(int i=0; i<mid; i++){
            thread1_set.add(set.pollFirst());
        }
        ...
        Thread thread1 = new Thread(new Runnable() {
            @Override
            public void run() {
                Iterator iterator = thread1_set.iterator();
                while (iterator.hasNext()){
                    Node node = (Node) iterator.next();
                    if(node.getIndegree()==0){
                        //入度为0的起始节点，直接用fnc()方法更新后续节点的入度值
                        fnc(node,node.getToSet());
                        continue;
                    }else {
                        path(lists1,new ArrayList<>(),node,node,1);
                        //起始节点查找一次后，就可移除
                        node.setIndegree(0);
                        fnc(node,node.getToSet());
                    }
            ...
            public void run() {
                ...
                while (iterator.hasNext()){
                    ...
                    if(node.getIndegree()==0){
                        ...
                        path(lists2,new ArrayList<>(),node,node,1);
                        //起始节点查找一次后，就可移除
                        node.setIndegree(0);
                        fnc(node,node.getToSet());
                    }
    ...
    private void fnc(Node start,Set<Node> set){
        Iterator iterator = set.iterator();
        while (iterator.hasNext()){
            Node node = (Node) iterator.next();
            if(node.getVal() < start.getVal()) {
                break;
            }else {
                //当前节点入度为1，可以移除，继续向下遍历
                if(node.getVal()==1) fnc(start,node.getToSet());
                //当前节点入度减一
